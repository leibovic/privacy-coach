...
const PREF_DONT_WARN_ENGINES = "extensions.privacycoach.dontWarnEngines";
...
XPCOMUtils.defineLazyGetter(this, "DEBUG", function() {
  try {
    return Services.prefs.getBoolPref(PREF_DEBUG);
  } catch (e) {
    // Set the pref so that it's easier to find in about:config.
    Services.prefs.setBoolPref(PREF_DEBUG, false);
    return false;
  }
});
...
let gRefreshBannerMenuId;
let gDebugBannerIndex = 0;
...
function loadIntoWindow(window) {
  ...
  // Add debugging menu item to see how banner message strings appear.
  // This will only work in Firefox 35+.
  if (DEBUG) {
    gRefreshBannerMenuId = window.NativeWindow.menu.add({
      name: "Show next banner",
      callback: () => {
        let message = gBannerMessages[gDebugBannerIndex];
        Messaging.sendRequest({
          type: "HomeBanner:Data",
          id: gDebugBannerIndex, // Fake the id, this means clicking on the banner won't work.
          text: message.text,
          iconURI: window.resolveGeckoURI(WHISTLE_ICON)
        });

        // Increment the banner index for the next time the user hits the menu item.
        gDebugBannerIndex++;
        if (gDebugBannerIndex === gBannerMessages.length) {
          gDebugBannerIndex = 0;
        }
      }
    });
...
function unloadFromWindow(window) {
  ...
  if (gRefreshBannerMenuId) {
    window.NativeWindow.menu.remove(gRefreshBannerMenuId);
  }
}
